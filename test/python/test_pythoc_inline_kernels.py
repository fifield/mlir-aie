...
"""Test Inline PythoC kernel compilation."""
...
@aie_kernel
def test_add_kernel(a: ptr[i32, True], b: ptr[i32, True], c: ptr[i32, True], N: i32):
...
def test_decorator():
    ...
    assert hasattr(
        test_add_kernel, "__aie_kernel__"
    ), "Missing __aie_kernel__ attribute"
    ...
    assert hasattr(
        test_add_kernel, "__aie_source__"
    ), "Missing __aie_source__ attribute"
    assert (
        "def test_add_kernel" in test_add_kernel.__aie_source__
    ), "Source code not captured"

    assert hasattr(
        test_add_kernel, "__aie_signature__"
    ), "Missing __aie_signature__ attribute"
...
def test_kernel_compilation():
    ...
    try:
        ...
        kernel = PythocKernel(test_add_kernel, [tile_ty, tile_ty, tile_ty, np.int32])
        ...
        print(f"  ✓ Object file size: {file_size} bytes")

        if file_size < 100:
            print(f"  ✗ Object file seems too small")
            return False

        # Verify companion LLVM IR exists for inspection
        ll_file = obj_file.with_suffix(".ll")
        if not ll_file.exists():
            print(f"  ✗ Companion LLVM IR file not found: {ll_file}")
            return False
...
def main():
    """Run all tests."""
    print("=" * 60)
    print("PythoC IRON Integration Tests")
    print("=" * 60)
