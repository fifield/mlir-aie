...
module {
  aie.device(npu1_1col) {
    ...
    // TRACE CONFIGURATION
    // ========================================================================
    // Using the new aie-insert-trace-flows pass:
    // - Add trace ops with events to capture
    // - The pass will automatically generate packet flows and routing
    // - No need to manually specify packet flows
    ...
    // NOTE: Packet flows are now automatically generated by aie-insert-trace-flows pass
    // No need to manually specify aie.packet_flow operations

    // ========================================================================
    // RUNTIME SEQUENCE WITH TRACE ACTIVATION
    // ========================================================================
    // The aie-insert-trace-flows pass will automatically inject trace setup
    // when a trace buffer argument is provided

    // Runtime sequence with trace buffer argument
    aie.runtime_sequence(
      %arg0: memref<4096xi32>,
      %arg1: memref<1xi32>,
      %arg2: memref<4096xi32>,
      %arg3: memref<1048576xi8>  // Trace buffer (1MB)
    ) {
      // NOTE: aie-insert-trace-flows pass will automatically inject:
      // - aie.trace.start_config for each trace
      // - Timer control writes
      // - BD configuration for trace buffer
      // - DMA setup for trace channel
      ...
      // NOTE: aie-insert-trace-flows pass will automatically inject trace stop
    }
  }
}
