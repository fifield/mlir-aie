...
def generate_trace_json(trace_buffer, mlir_file, json_file, verbosity=0):
    ...
    try:
        ...
        trace_events = parse_trace(trace_buffer, mlir_str, debug=(verbosity >= 2))

        # Write JSON output
        with open(json_file, "w") as f:
            json.dump(trace_events, f, indent=2)

        print(f"Trace JSON written to {json_file}")
        if verbosity >= 1:
...
def parse_args():
    ...
    parser.add_argument(
        "--trace-json", default="trace.json", help="Trace JSON output file"
    )
...
def main():
    ...
    for iter in range(num_iter):
        ...
        if args.trace_sz > 0 and iter == args.warmup:
            ...
            # Generate trace.json if MLIR file provided
            if args.mlir:
                generate_trace_json(
                    trace_buffer, args.mlir, args.trace_json, verbosity=args.verbosity
                )
            else:
                print("Skipping trace.json generation (no --mlir file specified)")
