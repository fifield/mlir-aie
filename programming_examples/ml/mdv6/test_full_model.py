...
def main():
    ...
    print("\nLoading NPU kernels...", end=" ", flush=True)
    # Kernel handles - grouped by layer type
    kh = {}
    for n in ['tf_elan_conv1','tf_elan_conv3x3','tf_elan_conv4',
              're4_conv1','re4_conv3x3','re4_conv4','re4_rn_conv1x1_64_32','re4_rn_conv3x3_32_32',
              're6_conv1','re6_conv3x3','re6_conv4','re6_rn_c1','re6_rn_c3','re6_rn_merge',
              're8_conv1','re8_conv3x3','re8_conv4','re8_rn_c1','re8_rn_c3','re8_rn_merge',
              'tf_aconv3','aconv5','aconv7','aconv16','aconv19',
              'spp_conv1',
              're12_conv1','re15_conv1','re15_conv4','re15_rn_merge',
              're18_conv1','re21_conv1']:
        kh[n] = ld(n)
    print("done")
    ...
    # Conv0 + Conv1 on CPU (stride=2 with BN+SiLU, needs fused tiled kernel for full NPU)
    print("  conv0+conv1...", end=" ", flush=True)
    with torch.no_grad():
        conv0 = model.conv0(x)
        conv1 = model.conv1(conv0)
    conv1_hwc = conv1.squeeze(0).permute(1, 2, 0).contiguous()
    print("CPU")

    # ELAN2 (160×160, 64→64)
    print("  elan2...", end=" ", flush=True)
    elan2 = run_elan(model.elan2, conv1_hwc, 160, 160, 64, 64,
                      kh['tf_elan_conv1'], kh['tf_elan_conv3x3'], kh['tf_elan_conv4'],
                      8, 64, 16, 32, 8, 64, 64, 32)
    print("NPU")

    # AConv3 + rep_elan4 [B3]
    print("  aconv3...", end=" ", flush=True)
    ac3 = run_aconv(kh['tf_aconv3'], model.aconv3, to_nchw(elan2), 80, 80, 128, 8, 16)
    print("NPU")
    print("  rep_elan4...", end=" ", flush=True)
    b3 = run_re(model.rep_elan4, ac3, 80, 80, 128, 128, 128, 64,
                kh['re4_conv1'], kh['re4_conv3x3'], kh['re4_conv4'],
                kh['re4_rn_conv1x1_64_32'], kh['re4_rn_conv3x3_32_32'], kh['tf_elan_conv1'],
    ...
    b3n = to_nchw(b3)
    ac5 = run_aconv(kh['aconv5'], model.aconv5, b3n, 40, 40, 192, 4, 8)
    print("NPU")
    print("  rep_elan6...", end=" ", flush=True)
    b4 = run_re(model.rep_elan6, ac5, 40, 40, 192, 192, 192, 96,
                kh['re6_conv1'], kh['re6_conv3x3'], kh['re6_conv4'],
                kh['re6_rn_c1'], kh['re6_rn_c3'], kh['re6_rn_merge'],
    ...
    b4n = to_nchw(b4)
    ac7 = run_aconv(kh['aconv7'], model.aconv7, b4n, 20, 20, 256, 4, 4)
    print("NPU")
    print("  rep_elan8...", end=" ", flush=True)
    b5 = run_re(model.rep_elan8, ac7, 20, 20, 256, 256, 256, 128,
                kh['re8_conv1'], kh['re8_conv3x3'], kh['re8_conv4'],
                kh['re8_rn_c1'], kh['re8_rn_c3'], kh['re8_rn_merge'],
                4, 32, 4, 16, 4, 16, 8, 64, 8, 16, 4, 32)
    print("NPU")

    # SPP9
    print("  spp9...", end=" ", flush=True)
    spp_c1 = run_tiled(kh['spp_conv1'], b5, fuse_bn(model.spp9.conv1), 20, 20, 128, 4, 4, 32, 1, 1, 0)
    ...
    n3 = run_tiled(kh['re8_conv4'], torch.cat(feats, dim=2), fuse_bn(model.spp9.conv5), 20, 20, 256, 4, 4, 16, 1, 1, 0)
    print("NPU")
    ...
    n4 = run_re(model.rep_elan12, cat12, 40, 40, 448, 192, 192, 96,
                kh['re12_conv1'], kh['re6_conv3x3'], kh['re6_conv4'],
                kh['re6_rn_c1'], kh['re6_rn_c3'], kh['re6_rn_merge'],
    ...
    p3 = run_re(model.rep_elan15, cat15, 80, 80, 320, 128, 128, 64,
                kh['re15_conv1'], kh['re4_conv3x3'], kh['re15_conv4'],
                kh['re4_rn_conv1x1_64_32'], kh['re4_rn_conv3x3_32_32'], kh['re15_rn_merge'],
    ...
    p3n = to_nchw(p3)
    ac16 = run_aconv(kh['aconv16'], model.aconv16, p3n, 40, 40, 96, 4, 8)
    ...
    p4 = run_re(model.rep_elan18, cat18, 40, 40, 288, 192, 192, 96,
                kh['re18_conv1'], kh['re6_conv3x3'], kh['re6_conv4'],
                kh['re6_rn_c1'], kh['re6_rn_c3'], kh['re6_rn_merge'],
                4, 32, 8, 16, 4, 32, 10, 48, 8, 16, 8, 48)
    print("NPU")
    ...
    p4n = to_nchw(p4)
    ac19 = run_aconv(kh['aconv19'], model.aconv19, p4n, 20, 20, 128, 4, 4)
    ...
    p5 = run_re(model.rep_elan21, cat21, 20, 20, 384, 256, 256, 128,
                kh['re21_conv1'], kh['re8_conv3x3'], kh['re8_conv4'],
                kh['re8_rn_c1'], kh['re8_rn_c3'], kh['re8_rn_merge'],
                4, 32, 4, 16, 4, 16, 8, 64, 8, 16, 4, 32)
    print("NPU")
