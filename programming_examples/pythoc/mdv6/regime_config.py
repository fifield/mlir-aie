...
CONV_REGIME_ARTIFACTS = {
    "regime_shared_conv3x3": ConvRegimeArtifact(
        regime="R1-R3",
        xclbin_name="regime_shared_conv3x3",
        kernel_size=3,
        stride=1,
        tile_h=4,
        tile_w=4,
        ...
        input_depth=1,
        members={
            "mc_elan_c3": (4, 4, 32, 16, 1, 1, 4),
            "mc_re4_c3": (4, 4, 64, 16, 1, 1, 2),
            "mc_re4_rn3": (4, 4, 32, 16, 1, 1, 4),
            "mc_re6_c3": (4, 4, 96, 16, 1, 1, 1),
            "mc_re6_rn3": (4, 4, 48, 16, 1, 1, 1),
            "mc_re8_c3": (4, 4, 128, 16, 1, 1, 1),
            "mc_re8_rn3": (4, 4, 64, 16, 1, 1, 1),
        },
    ),
    "regime_r1_conv3x3": ConvRegimeArtifact(
        regime="R1",
        xclbin_name="regime_r1_conv3x3",
        kernel_size=3,
        stride=1,
        tile_h=8,
        tile_w=8,
        ...
        input_depth=1,
        members={
            # name: active tile_h, tile_w, ic, oc_block, stride, padding, standalone ppc
            "mc_elan_c3": (8, 8, 32, 32, 1, 1, 4),
            "mc_re4_c3": (8, 8, 64, 16, 1, 1, 2),
            "mc_re4_rn3": (8, 8, 32, 32, 1, 1, 4),
        },
    ),
...
GEMM_REGIME_ARTIFACTS = {
    ...
    "regime_shared_gemm_kblocked": GemmRegimeArtifact(
        regime="R4",
        xclbin_name="regime_shared_gemm_kblocked",
        tile_m=24,
        ic=512,
        oc=256,
        patches_per_core=2,
...
def _is_skipped_regime(artifact) -> bool:
    """Regimes that are *never* dispatched, even as fallback.

    R5 (`regime_r5_stride2_conv3x3`) was added after the legacy `2ce24156e`
    Tier-2 sweet-spot checkpoint and aggressively retiles stride-2 stems to
    4×4/OC=8. PERF_PLAN.md labels it "useful context-collapse evidence but
    not a performance-neutral route" — launches roughly double on stem
    layers. Skipping it lets stems/aconv* fall through to per-shape
    dispatch (`mc_ftconv0`, `mc_aconv5`, …), which is what the sweet spot
    expects.
    """
    return getattr(artifact, "regime", "") == "R5"


def _is_fallback_regime(artifact) -> bool:
    """`regime_shared_*` envelopes unify multiple tiers under one xclbin and
    regress wall ~2× (ppc/tile underfill, PERF_PLAN.md). Kept on disk as
    proof-of-context-collapse — dispatched only when no per-tier covers the
    layer at all (which doesn't happen for any MDV6 layer right now)."""
    return artifact.xclbin_name.startswith("regime_shared_")


def conv_regime_for_layer(layer_name: str):
    """Lookup with per-tier preferred, most-specific tier first.

    Iteration order: reversed dict → R3 → R2 → R1 (per-tier) → shared. R5
    is skipped entirely so stems/aconv* fall through to per-shape dispatch
    (the sweet-spot config). Shared envelopes are kept only as a last
    fallback when no per-tier lists the layer.
    """
    fallback = None
    for artifact in reversed(list(CONV_REGIME_ARTIFACTS.values())):
        if layer_name not in artifact.members:
            continue
        if _is_skipped_regime(artifact):
            continue
        if _is_fallback_regime(artifact):
...
def gemm_regime_for_layer(layer_name: str, ic=None, oc=None, k_block=None):
    """Same most-specific-tier-first policy for GEMM artifacts."""
    fallback = None
    for artifact in reversed(list(GEMM_REGIME_ARTIFACTS.values())):
