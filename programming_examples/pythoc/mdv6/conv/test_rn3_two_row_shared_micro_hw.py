#!/usr/bin/env python3
"""Two-row shared-intermediate rn3pair hardware microtest.

This tests a lower-pressure combined conv1+conv2 shared-intermediate design:
for each pair of final output rows, keep only array[f32,64] accumulators live while
reusing array[f32,100] for the conv1 midplane of one mid channel at a time.
...
@aie_kernel
def rn3_two_row_shared_bf16(input_patch: ptr[bf16, True], weights: ptr[bf16, True], output: ptr[bf16, True], ic: i32, mid: i32) -> void:
    ...
    accrows: array[f32, 64] = array[f32, 64]()
    patch_w: i32 = 12
    ...
    while rb < 8:
        ...
        o1: i32 = 0
        while o1 < mid:
            # Compute full 10x10 conv1 midplane for this mid channel.
            mr: i32 = 0
            while mr < 10:
                mc: i32 = 0
                while mc < 10:
                    ...
                    while kh1 < 3:
                        ...
                        while kw1 < 3:
                            i: i32 = 0
                            while i < ic:
                                in_idx: i32 = ((mr + kh1) * patch_w + (mc + kw1)) * ic + i
                    ...
                    if ax1 < 0.0:
                        ax1 = -x1
                    midplane[mr * 10 + mc] = x1 * (0.5 + x1 / (2.0 + 2.0 * ax1))
                    mc = mc + 1
                mr = mr + 1
            ...
            while rr < 2:
                ...
                while c < 8:
                    ...
                    while kh2 < 3:
                        kw2: i32 = 0
                        while kw2 < 3:
                            y1: f32 = midplane[(rb + rr + kh2) * 10 + (c + kw2)]
                            w2_base: i32 = w2_off + ((o1 * 3 + kh2) * 3 + kw2)
                            aidx: i32 = (rr * 8 + c) * 4
                            accrows[aidx] = accrows[aidx] + y1 * f32(weights[w2_base])
                            accrows[aidx + 1] = accrows[aidx + 1] + y1 * f32(weights[w2_base + mid * 9])
                            accrows[aidx + 2] = accrows[aidx + 2] + y1 * f32(weights[w2_base + 2 * mid * 9])
                            accrows[aidx + 3] = accrows[aidx + 3] + y1 * f32(weights[w2_base + 3 * mid * 9])
                            kw2 = kw2 + 1
                        kh2 = kh2 + 1
                    c = c + 1
                rr = rr + 1
        ...
        while rr2 < 2:
            ...
            while c2 < 8:
                o2: i32 = 0
                while o2 < 4:
                    idx: i32 = (rr2 * 8 + c2) * 4 + o2
                    x2: f32 = accrows[idx] * f32(weights[bn2_w_off + o2]) + f32(weights[bn2_b_off + o2])
                    ax2: f32 = x2
                    if ax2 < 0.0:
                        ax2 = -x2
                    output[((rb + rr2) * 8 + c2) * 4 + o2] = bf16(x2 * (0.5 + x2 / (2.0 + 2.0 * ax2)))
                    o2 = o2 + 1
                c2 = c2 + 1
            rr2 = rr2 + 1
        rb = rb + 2
...
def build_module(ic=48, mid=48, stack_size=4096):
    ...
    kernel = PythocKernel(
        rn3_two_row_shared_bf16,
        [input_ty, weight_ty, output_ty, np.int32, np.int32],
        extra_globals={"array": array},
    )
    in_fifo = ObjectFifo(input_ty, depth=1, name="rn3_two_in")
    wt_fifo = ObjectFifo(weight_ty, depth=1, name="rn3_two_wt")
    out_fifo = ObjectFifo(output_ty, depth=1, name="rn3_two_out")
    ...
    with rt.sequence(input_ty, weight_ty, output_ty) as (A, W, C):
        ...
        rt.fill(wt_fifo.prod(), W, TensorAccessPattern((wt_len,), offset=0, sizes=[1, wt_len], strides=[0, 1]))
...
def main(argv=None):
    ...
    p.add_argument("--workdir", default="conv/build_rn3_two_row_shared_micro")
    ...
    print(f"built rn3_two_row_shared ic={args.ic} mid={args.mid} stack={args.stack_size} mlir={mlir}")
    if args.build_only:
        return 0
    got = run_kernel(xclbin, insts, args.ic, args.mid)
    print(f"PASS: rn3 two-two-row-shared micro ic={args.ic} mid={args.mid} first={got.reshape(-1)[:8].tolist()}")
    return 0
