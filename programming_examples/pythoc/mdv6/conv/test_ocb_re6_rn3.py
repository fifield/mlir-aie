#!/usr/bin/env python3
"""Bytewise correctness check for ocb_re6_rn3.elf.

Tests OCB-unrolled re6_rn3 (n_ocb=3, ppc=4) vs n_ocb=1 ppc=4 reference
run 3 times with different weights. The ppc=4 absorbs the layer's
4 spatial batches into one xrt.run via compile-time unroll.
"""
import os
import sys
import time
...
_OCB_ELF = os.path.join(_BD, "ocb_re6_rn3_x1.elf")
_REF_ELF = os.path.join(_BD, "ocb_re6_rn3_ref_x1.elf")

# re6_rn3 OCB-unroll: tile 4×4, ic=48, oc_block=16, n_ocb=3, ppc=4
TILE_H = TILE_W = 4
IC = 48
OC_BLOCK = 16
N_OCB = 3
...
PATCH_H = (TILE_H - 1) + KS  # 6
PATCH_RAW = PATCH_H * PATCH_H * IC  # 1728
PATCH_SIZE = PATCH_RAW + (PATCH_RAW % 2)  # 1728
CORE_INPUT = PPC * PATCH_SIZE       # 6912
HOST_INPUT = N_CORES * CORE_INPUT   # 221184
WEIGHT_SLOT = OC_BLOCK * IC * KS * KS + 2 * OC_BLOCK  # 6944
OUTPUT_TILE = TILE_H * TILE_W * OC_BLOCK  # 256
CORE_OUTPUT = PPC * OUTPUT_TILE     # 1024
HOST_OUTPUT = N_CORES * CORE_OUTPUT  # 32768
...
def main():
    ...
    # OCB ELF: one xrt.run, big_W = [wt0, wt1, wt2]
    ...
    print(f"Dispatching OCB-unrolled ELF (one xrt.run, {N_OCB} OCBs × {PPC} ppc)...")
    ...
    print(f"  ocb wall: {ocb_ms:.2f} ms; nonzero: {nz}/{len(big_O)}")

    # Reference: n_ocb=1 ELF run 3 times
    ...
    for ocb_idx, wt in enumerate(wts):
        ...
        total_ref_ms += dt
        refs.append(_bo_read(ref_O_bo, HOST_OUTPUT))
        print(f"  ref OCB{ocb_idx}: {dt:.2f} ms")
    ...
    print(f"  Speedup:                     {total_ref_ms/ocb_ms:.2f}x")
