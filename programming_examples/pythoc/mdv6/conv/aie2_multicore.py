...
def _parse_args(argv):
    ...
    # Hardware trace: enable per-worker tracing. Only meaningful when invoked
    # via build_merged.py with --trace; the dispatcher's last sub-device arg
    # is the OUT BO, so trace bytes get appended there (ddr_id=-1).
    parser.add_argument("--trace-size", type=int, default=0,
                        help="trace buffer size in bytes (0 disables trace)")
    parser.add_argument("--trace-worker", type=int, default=0,
                        help="worker index to trace (0..n_cores-1)")
...
if __name__ == "__main__":
    ...
    module = multicore_conv(
        ...
        trace_worker=args.trace_worker,
    )
    print(module)
