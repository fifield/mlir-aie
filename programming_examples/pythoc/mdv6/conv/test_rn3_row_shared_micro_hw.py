#!/usr/bin/env python3
"""Row-sliced shared-intermediate rn3pair hardware microtest.

This tests a lower-pressure combined conv1+conv2 shared-intermediate design:
for each final output row, keep only array[f32,32] accumulators live while
...
@aie_kernel
def rn3_row_shared_bf16(input_patch: ptr[bf16, True], weights: ptr[bf16, True], output: ptr[bf16, True], ic: i32, mid: i32) -> void:
        ...
        while o1 < mid:
            ...
            while c < 8:
                ...
                while kh2 < 3:
                    kw2: i32 = 0
                    while kw2 < 3:
                        y1: f32 = midplane[(r + kh2) * 10 + (c + kw2)]
...
def build_module(ic=48, mid=48, stack_size=4096):
    ...
    in_fifo = ObjectFifo(input_ty, depth=1, name="rn3_row_in")
    wt_fifo = ObjectFifo(weight_ty, depth=1, name="rn3_row_wt")
    out_fifo = ObjectFifo(output_ty, depth=1, name="rn3_row_out")
...
def main(argv=None):
    ...
    p.add_argument("--workdir", default="conv/build_rn3_row_shared_micro")
    ...
    print(f"built rn3_row_shared ic={args.ic} mid={args.mid} stack={args.stack_size} mlir={mlir}")
    if args.build_only:
        return 0
    got = run_kernel(xclbin, insts, args.ic, args.mid)
    print(f"PASS: rn3 row-shared micro ic={args.ic} mid={args.mid} first={got.reshape(-1)[:8].tolist()}")
    return 0
