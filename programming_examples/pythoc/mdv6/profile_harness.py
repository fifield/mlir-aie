...
class Profiler:
    ...
    def _install_hooks(self):
        ...
        # Merged-ELF dispatch (mcr._xrt_run_kernel) is patched further below,
        # after the mcr module is imported.
        ...
        # Pack/repack functions in run_tiled_mc.py. Use the mcr instance
        # already loaded by the running test script (sys.modules["__main__"])
        # — it has its own private mcr via spec_from_file_location. Patching
        # *that* instance is what reaches the real dispatch path. Loading a
        # fresh mcr_prof here (the previous approach) created a parallel copy
        # that nothing actually called.
        main_mod = sys.modules.get("__main__")
        mcr = getattr(main_mod, "mcr", None) if main_mod is not None else None
        if mcr is None:
        ...
        # Hook _xrt_run_kernel — the module-level seam every merged-ELF
        # dispatch flows through (added Phase A.2 follow-up). Same counters
        # as the DefaultNPURuntime.run wrapper so launches/npu_run merge
        # cleanly when both paths are active.
        if hasattr(mcr, "_xrt_run_kernel"):
            _orig_xrt = mcr._xrt_run_kernel

            def _wrap_xrt(kernel, args, *a, **kw):
                now = time.perf_counter()
                if prof._last_launch_end is not None:
                    gap = now - prof._last_launch_end
                    prof._cur.inter_launch_gaps_ms += gap * 1000
                t0 = time.perf_counter()
                r = _orig_xrt(kernel, args, *a, **kw)
                t1 = time.perf_counter()
                prof._bump("npu_run", t1 - t0)
                prof._cur.n_launches += 1
                prof._last_launch_end = t1
        ...
        ett_spec = importlib.util.spec_from_file_location(
            "ett_prof", os.path.join(_base, "_full_model_helpers", "elan_test_tiled.py"))
        ...
        # Make test_full_model_mc.py see the wrapped versions. Patch both the
        # running script (sys.modules["__main__"]) and the importable copy in
        # sys.modules["test_full_model_mc"] — Python keeps these distinct, and
        # only the former is what the user actually executes.
        tfmm_targets = []
        main_mod = sys.modules.get("__main__")
        if main_mod is not None and hasattr(main_mod, "run_tiled_mc"):
            tfmm_targets.append(main_mod)
        try:
            import test_full_model_mc as tfmm
            tfmm_targets.append(tfmm)
        except ImportError:
            pass
    ...
    def report(self, baseline_path: str | None = None,
        ...
        print("-" * 78)
        rows = [
            ("npu_run", npu, "NPU active (DefaultNPURuntime.run + merged-ELF xrt.run)"),
