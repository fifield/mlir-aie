...
| `builders/o_ffn.py` | prefill (O + FFN with GEMMs) | per-layer prefill | ◐ Phase 4.6b (3 of 9 devices placed, cached-splice) |

Phase 4.6b: 3 of 9 devices on placed-IRON (`rm_weighted_rms_norm_seg`,
`ra_add_seg`, `fa_add_seg`); other 6 devices via splice. Subsequent
phases 4.6c-f extend the splice to remaining devices.
