...
def awq_gemv_cpu_reference(
    x_bf16: np.ndarray,
    awq: AwqLinear,
    *,
    dtype=bfloat16,
) -> np.ndarray:
    """Compute one repacked-AWQ GEMV directly from packed uint4 tensors.

    This is the correctness-first runtime/API seam for the fused AWQ GEMV path:
    it consumes ``AwqLinear.qweight`` and ``AwqLinear.params`` directly and does
    not materialize the full ``(m, k)`` dequantized weight matrix. Accumulation is
    float32; the final vector is cast to ``dtype`` (BF16 by default for existing
    decode-compatible buffers, or float32 in tests/kernel oracles).
    """
    _validate_awq_linear_shape(awq)
    x = np.asarray(x_bf16, dtype=np.float32)
    if x.ndim != 1:
        raise ValueError(f"AWQ GEMV input must be 1D, got shape {x.shape}")
    if x.shape[0] != awq.k:
        raise ValueError(f"input length {x.shape[0]} does not match AWQ K={awq.k}")

    qweight = np.asarray(awq.qweight, dtype=np.uint8)
    params = np.asarray(awq.params, dtype=np.float32)
    out = np.zeros(awq.m, dtype=np.float32)
    for m_idx in range(awq.m):
        acc = np.float32(0.0)
        for k_idx in range(awq.k):
            packed = int(qweight[m_idx, k_idx // 2])
            if k_idx & 1:
                q = (packed >> 4) & 0xF
            else:
                q = packed & 0xF
            group = k_idx // awq.group_size
            scale = params[m_idx, 2 * group]
            zero = params[m_idx, 2 * group + 1]
            acc += x[k_idx] * ((np.float32(q) - zero) * scale)
        out[m_idx] = acc
