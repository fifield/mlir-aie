...
# ---------------------------------------------------------------------------
def _emit_awq_qkv_rope_pack(sym: str = RGR2_PACK_SYM, fold_rms: bool = False) -> None:
    ...
    ``fold_rms=True`` (pack_mode ``rgr1_ddr``) absorbs the standalone
    ``r_rms_awq_seg`` device as one more tile (col 0, row 3) so the dispatcher
    has a single device run -- the exact AWQ counterpart of the BF16
    ``_emit_qkv_rope_pack(fold_rms=True)`` fold. The RMSNorm stays BF16
    (``rms_norm_2048_bf16``); only the Q/K/V matvecs are AWQ uint4. Same
    arg->buffer mapping and DDR handoff as the standalone (arg0->buf_w,
    arg1->buf_x, buf_y->arg2); the herd reads the normed vector back from DDR
    (arg2), with the RMS run awaited before the matvec broadcast.
    """
    mat_chans = _CHANNELS["q_matvec_awq_bf16_0"]
    rq_chans = _CHANNELS["rq_rope_awq_seg"]
    rk_chans = _CHANNELS["rk_rope_awq_seg"]
    rms_chans = _CHANNELS["r_rms_awq_seg"]
    row_bytes = ROW_BYTES  # 1088
    ...
    def _dev():
        ...
        # RMS tile (fold_rms): BF16 RMSNorm, copied from the standalone
        # r_rms_awq_seg. 6 locks (ids 5..0) + 4 L1 bf16 buffers.
        rms_locks = None
        rms_bufs = None
        if fold_rms:
            _BF16_2048_L1 = bf16_memref(EMB_DIM, memory_space=2)
        ...
        # RMS tile (fold_rms): BF16, copy of the standalone r_rms_awq_seg.
        if fold_rms:
            _l = rms_locks
            _b = rms_bufs
        ...
        # RMS flows (fold_rms): spare shim channels (shim1 MM2S1/S2MM1,
        # shim4 MM2S1), unused by the herd, the broadcast, and RoPE.
        if fold_rms:
        ...
        if fold_rms:
            # in0 (id 0) <- arg1 (x); in1 (id 1) <- arg0 (weight);
            # out (id 2) -> arg2 (normed broadcast input read by the herd).
        ...
        def _seq(*args):
            ...
            if fold_rms:
                # RMS first: BF16 RMSNorm writes normed (arg2) to DDR, awaited
                # before the herd reads it back via the input broadcast.
                # in0<-arg1 (x), in1<-arg0 (weight), out->arg2 (normed).
...
def build_rms_gemv_rope_awq_module(emb_dim: int = EMB_DIM,
    ...
    with mlir_mod_ctx() as ctx:
        if pack_mode == "rgr1_ddr":
            # Single device: RMS folded into the AWQ Q/K/V+RoPE pack.
            _emit_awq_qkv_rope_pack(fold_rms=True)
            ...
            _emit_awq_matvec_seg("q_matvec_awq_bf16_0",
                                  ...
                                  out_rows=EMB_DIM)
        # rgr1_ddr absorbs RMS into the pack device; no standalone r_rms_awq_seg.
        if pack_mode != "rgr1_ddr":
            _emit_r_rms_seg()
        if pack_mode == "rgr1_ddr":
            dispatch_sequence = (RGR2_PACK_SYM,)
        elif pack_mode == "rgr2_ddr":
            dispatch_sequence = ("r_rms_awq_seg", RGR2_PACK_SYM)
        else:
            dispatch_sequence = DEFAULT_DISPATCH_SEQUENCE
