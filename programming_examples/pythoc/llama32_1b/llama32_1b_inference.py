...
class Session:
    ...
    awq_decode_experimental: bool = False
...
def prepare_runtime(
    ...
    if awq_decode_experimental and getattr(weights, "awq_layers", None):
        _preload_decode_weights_awq(decode_cache, weights, config)
...
def generate(
    ...
    if awq_decode_experimental and not getattr(weights, "awq_layers", None):
        raise ValueError("awq_decode_experimental requires weights loaded with --quant awq")

    if not streaming:
        print(f"\n{'='*60}")
        print(f"LLAMA Inference: prompt_len={seq_len}, n_tokens={n_tokens}")
        if awq_decode_experimental:
            print("Experimental AWQ decode: using direct packed-AWQ NPU GEMV for o_proj+FFN")
        print(f"{'='*60}\n")
    ...
    for token_idx in range(n_tokens):
        ...
        for layer_idx in range(config.n_layers):
            x = run_decode_block(
                ...
                rope_lut_bf16,
                awq_layer_weights=(
                    weights.awq_layers[layer_idx] if awq_decode_experimental else None
                ),
            )
...
def build_session(args) -> Session:
    ...
    prepare_runtime(
        ...
        awq_decode_experimental=args.awq_decode_experimental,
    )
...
def run_once(
    ...
    *,
    ...
    awq_decode_experimental: Optional[bool] = None,
) -> tuple[list, int]:
    ...
    if awq_decode_experimental is None:
        awq_decode_experimental = session.awq_decode_experimental
    ...
    generated = generate(
        ...
        cpu_attn=cpu_attn,
        on_token=on_token,
        awq_decode_experimental=awq_decode_experimental,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Opt-in experimental decode path: keep q/k/v on existing BF16 kernel, "
    ...
    if args.awq_decode_experimental and args.quant != "awq":
        parser.error("--awq-decode-experimental requires --quant awq")
