...
buildObjectSubgraph(EdgeWithTypedOutput<ModRef> &lowered,
  ...
  auto &peanoLinked =
      bundle(peanoCompat.out, irLinkFiles.out)
          .map<File>(
              ...
              [llvmLinkCmd](const Item<std::string> &ir,
                ...
                if (links.get().empty()) {
                  ...
                  // input. Copy it to this edge's own output path -- aliasing
                  // the peano-compat item's path collides with it (the engine
                  // requires each item's output path to be unique).
                  if (std::error_code ec =
                          llvm::sys::fs::copy_file(ir.asFile(), out.filePath)) {
                    llvm::errs()
                        << "aiecc: peano-linked: cannot copy '" << ir.asFile()
                        << "' to '" << out.filePath << "': " << ec.message()
                        << "\n";
                    return mlir::failure();
                  }
                  out.value = File{};
          ...
          .threadSafe();
  auto &opted =
      peanoLinked.map<File>("opted_{0}.ll", optCmd).threadSafe();
