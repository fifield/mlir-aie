...
class AIEAddressDecoder:
    ...
    def parse_address(self, address: int) -> Optional[Dict]:
        ...
        - col: column number
        - row: row number
          - row 0: shim tile (noc, pl modules)
          - row 1: memtile (memory_tile module)
          - row 2+: core tile (core, memory modules)
        """

        # Extract col, row, and offset using bit shifts
        col = (address >> 25) & 0x7F  # bits 25-31 (7 bits for column)
        row = (address >> 20) & 0x1F  # bits 20-24 (5 bits for row)
        register_offset = address & 0xFFFFF  # bits 0-19 (offset within tile)

        # Determine tile type based on row
        if row == 0:
            tile_type = "shim_tile"
            module_candidates = ["noc", "pl"]
        elif row == 1:
    ...
    def encode_address(
        ...
        # Calculate address
        address = ((col << 25) | (row << 20)) + register_offset
...
class MLIRModuleAnnotator:
    ...
    def annotate_operation(self, op, target_model) -> bool:
        ...
        if row is not None and col is not None:
            # Construct full address: ((col << 25) | (row << 20)) + offset
            full_address = ((col << 25) | (row << 20)) + address
        else:
