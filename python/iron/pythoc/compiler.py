...
def _get_llc_path() -> str:
    ...
    default_llvm_aie = Path('/scratch/jefff/acdc/aie-venv/lib/python3.12/site-packages/llvm-aie/bin/llc')
...
def compile_pythoc_source(
    ...
    try:
        ...
        try:
            import pythoc
            from pythoc import (
                compile, ptr, i8, i16, i32, i64, u8, u16, u32, u64,
                f16, f32, f64, bf16, void, bool as pbool
            )
            from pythoc.aie import (
                aie_vector, load_v, store_v, vector_add, vector_mul, vector_sub,
                zeros, broadcast, vector_max, vector_min, vector_lt, vector_gt,
                vector_eq, vector_select, extract_elem, insert_elem, shift_bytes,
                vector_and, vector_or, concat, filter_even, filter_odd,
                interleave_unzip, accumulate, accum_to_vector, replicate_4x
            )
            ...
            user_globals.update({
                'compile': compile,
                'ptr': ptr, 'i8': i8, 'i16': i16, 'i32': i32, 'i64': i64,
                'u8': u8, 'u16': u16, 'u32': u32, 'u64': u64,
                'f16': f16, 'f32': f32, 'f64': f64, 'bf16': bf16,
                'void': void, 'bool': pbool,
                'aie_vector': aie_vector,
                'load_v': load_v, 'store_v': store_v,
                'vector_add': vector_add, 'vector_mul': vector_mul, 'vector_sub': vector_sub,
                'zeros': zeros, 'broadcast': broadcast,
                'vector_max': vector_max, 'vector_min': vector_min,
                'vector_lt': vector_lt, 'vector_gt': vector_gt, 'vector_eq': vector_eq,
                'vector_select': vector_select,
                'extract_elem': extract_elem, 'insert_elem': insert_elem,
                'shift_bytes': shift_bytes,
                'vector_and': vector_and, 'vector_or': vector_or,
                'concat': concat, 'filter_even': filter_even, 'filter_odd': filter_odd,
                'interleave_unzip': interleave_unzip,
                'accumulate': accumulate, 'accum_to_vector': accum_to_vector,
        ...
        # For Phase 1, we return the .ll file
        # IRON's compilation infrastructure will handle .ll → .o conversion
        # using the Peano toolchain (similar to how it handles C++ kernels)
        if verbose:
            print(f"Returning LLVM IR file (object compilation handled by IRON)")
