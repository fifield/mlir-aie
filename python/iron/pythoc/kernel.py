...
class PythocKernel(Kernel):
    ...
    Inline (decorated-function) kernels are compiled on construction; by
    default they yield a .o that is object-linked into the core. Passing
    ``inline=True`` instead emits an ``alwaysinline`` ``.ll`` that aiecc
    llvm-links into the core module and inlines — eliminating the
    ``func.call`` and the separate kernel ``.o``. (Requires an aiecc that
    treats ``.ll``/``.bc`` link entries as IR-merge artifacts.)
    ...
    def __init__(
        ...
        Args:
            ...
            inline: When True (decorated-function mode only), compile the
                     kernel to an ``alwaysinline`` ``.ll`` that aiecc merges
                     into the core via llvm-link, instead of a separately
                     object-linked ``.o``. The merged kernel is inlined into
                     the core body (no surviving ``func.call``).
        ...
        if isinstance(kernel_fn_or_name, str):
            ...
            # Compile the inline function. With inline=True this returns an
            # alwaysinline .ll (IR-merge artifact); otherwise a compiled .o.
